...
public class Dummies_user_reg {
	...
	//private static date_class[] fecha_cont = { new date_class("03/04/2000", ""), new date_class("18/11/2003",""), new date_class("12/3/2014",""),
			//new date_class("23/9/2009","4"), new date_class("25/4/2014","5")};
	private static int[] actividad = {0, 5, 15, 25, 144};
	private static int[] punts = {0, 5, 15, 25, 144};
        private static String[] karma = {"0", "5", "15", "25", "144"};
	...
	public static void Load_Dummies() {
		...
		for (int i = 0; i < LONGDUMMIES; i++) { //
			...
			u = new User_reg(dni[i], nom[i], cognom[i], date_birthday[i], mobil[i], email[i], avatar[i], user[i], pass[i], status[i], actividad[i], karma[i], punts[i]);
